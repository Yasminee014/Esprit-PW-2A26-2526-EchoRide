...
require_once __DIR__ . '/../modele/LostFoundPredictionService.php';
...
final class LostFoundAdminController
{
    ...
    private LostFoundPredictionService $predictionService;

    public function __construct(private readonly LostFoundRepository $repository)
    {
        $this->predictionService = new LostFoundPredictionService();
    }
    ...
    public function listWithPrediction(): array
    {
        $declarations = $this->repository->findAll();
        $comments = $this->repository->findAllComments();

        return $this->predictionService->enrichDeclarations($declarations, $comments);
    }
    ...
    public function create(array $input): array
    {
        ...
        $this->sendNotification('Nouvelle declaration publiee', 'cree', $id, [
            'titre' => (string) $input['titre'],
            'description' => (string) $input['description'],
            'categorie' => (string) $input['categorie'],
            'lieu_perte' => isset($input['lieu_perte']) ? (string) $input['lieu_perte'] : '',
            'date_perte' => (string) $input['date_perte'],
    ...
    public function changeStatus(int $id, string $status): array
    {
        ...
        $updated = $this->repository->updateStatus($id, $status);

        if ($updated) {
            $row = $this->repository->findById($id);
            if ($row !== null) {
                $this->sendNotification('Statut de declaration mis a jour', 'mise a jour statut', $id, $row, null);
            }
        }
