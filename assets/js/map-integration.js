...
function mapNotify(message, ok = true) {
...
async function handleMapClick(latlng) {
  if (!_mapState.mode) {
    mapNotify('Sélectionnez d\'abord un mode (Départ / Destination / Arrêt)', false);
    return;
  }
...
function applyMapToForm() {
  ...
  closeMapModal();
  mapNotify('Points appliqués au formulaire ✓', true);
}
...
document.body.style.paddingRight = '0px';
 ...
 const trip    = (typeof window.allTrips !== 'undefined') ? window.allTrips.find(t => t.id_T == tripId) : null;
 const allD    = (typeof window.allDests !== 'undefined') ? window.allDests : [];
  ...
  loadLeaflet(() => {
    ...
    if (markers.length === 0) {
      mapNotify('Coordonnées des villes non disponibles', false);
    }
  });
}
...
function patchAllTripsRender() {
  // Le rendu de "Tous les trajets" est géré par user.php.
  // Ne plus surcharger loadAllTrips ici pour éviter les conflits JS.
}

function getTripIdFromRowAction(btn) {
  const row = btn.closest('tr');
  if (!row) return 0;
  const chip = row.querySelector('.chip');
  if (!chip) return 0;
  const m = (chip.textContent || '').match(/#\s*(\d+)/);
  return m ? parseInt(m[1], 10) : 0;
}

function ensureAllTripsMapButtons() {
  const tbody = document.getElementById('allTripsBody');
  if (!tbody) return;

  tbody.querySelectorAll('tr').forEach(row => {
    const actions = row.querySelector('.abtns');
    if (!actions) return;
    const hasMapBtn = actions.querySelector('.abtn-map');
    if (hasMapBtn) return;

    const tripId = getTripIdFromRowAction(actions);
    if (!tripId) return;

    const mapBtn = document.createElement('button');
    mapBtn.className = 'abtn abtn-map';
    mapBtn.type = 'button';
    mapBtn.title = 'Voir sur la carte';
    mapBtn.setAttribute('data-trip-id', String(tripId));
    mapBtn.innerHTML = '<i class="fas fa-map-marked-alt"></i>';
    actions.insertBefore(mapBtn, actions.firstChild);
  });
}

function bindAllTripsMapClick() {
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('.abtn-map');
    if (!btn) return;

    const directId = parseInt(btn.getAttribute('data-trip-id') || '0', 10);
    const rowId = getTripIdFromRowAction(btn);
    const tripId = directId > 0 ? directId : rowId;
    if (!tripId) return;

    e.preventDefault();
    openTripViewMap(tripId);
  });
}
...
function initMapIntegration() {
  ...
  exposeGlobals();
  patchAllTripsRender();
  bindAllTripsMapClick();
  ensureAllTripsMapButtons();
