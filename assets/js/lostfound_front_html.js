...
const DECLARANTS_NON_INSCRITS_KEY = 'declarants_non_inscrits';
    ...
    function seedDemoDataIfNeeded() {
      ...
      if (hasObjets && hasSignalements) {
        return;
      }

      const demoObjets = [
        { id: 11, title: 'Sac a dos noir', description: 'Sac a dos noir avec chargeur et cahier bleu.', categorie: 'bagage', photo_url: '', date_perte: '2026-04-10', statut: 'perdu', trajet_id: 201, passager_id: 1, anonyme_nom: null, lieu_perte: 'Gare centrale' },
        { id: 12, title: 'Telephone noir', description: 'iPhone noir dans une coque transparente.', categorie: 'electronique', photo_url: '', date_perte: '2026-04-11', statut: 'retrouve', trajet_id: 202, passager_id: null, anonyme_nom: 'Nora K.', lieu_perte: 'Campus Nord' },
        { id: 13, title: 'Carte etudiante', description: 'Carte etudiante au nom de Camille Bernard.', categorie: 'document', photo_url: '', date_perte: '2026-04-09', statut: 'perdu', trajet_id: 203, passager_id: 3, anonyme_nom: null, lieu_perte: 'Centre ville' }
      ];

      const demoSignalements = [
        { id: 101, message: 'Objet repere a l arriere du vehicule', date_signalement: '2026-04-10T13:14:00', conducteur_id: 27, objet_id: 11 },
        { id: 102, message: 'Telephone retrouve sous le siege passager', date_signalement: '2026-04-11T19:21:00', conducteur_id: 28, objet_id: 12 }
      ];
    ...
    function editPost(id) {
      ...
      obj.title = normalizedTitle;
      obj.description = normalizedDescription;
      obj.categorie = normalizedCategory;
      obj.lieu_perte = normalizedPlace;
      obj.date_perte = normalizedDate;
    ...
    function removePost(id) {
      ...
      objets = objets.filter((row) => Number(row.id) !== Number(id));
      signalements = signalements.filter((row) => Number(row.objet_id) !== Number(id));
      saveObjets();
      saveSignalements();
      activePostId = objets.length ? Number(objets[0].id) : 0;
      renderAll();
    }
    ...
    function bindPublishForm() {
      ...
      els.publishForm.addEventListener('submit', async (e) => {
        ...
        const place = document.getElementById('place').value.trim();
        const trajetIdValue = document.getElementById('trajetId').value.trim();
        const trajetId = Number(trajetIdValue);
        ...
        if (!Number.isInteger(trajetId) || trajetId < 1) {
          errors.trajetId = 'Trajet ID invalide (entier positif attendu).';
        }
        ...
        const newObj = {
          ...
          statut: objectStatus,
          trajet_id: trajetId,
          passager_id: selectedDeclarantNonInscrit ? null : currentUserId,
          anonyme_nom: selectedDeclarantNonInscrit ? selectedDeclarantNonInscrit : null,
          lieu_perte: place
        };
        ...
        if (selectedDeclarantNonInscrit) {
          serverPayload.anonyme_nom = selectedDeclarantNonInscrit;
        } else {
          serverPayload.passager_id = currentUserId;
          serverPayload.user_id = currentUserId;
        }

        submitServerAction('create_declaration', serverPayload);
      });
    ...
    function bindModals() {
      ...
      document.getElementById('closePublicationsImage').addEventListener('click', () => els.publicationsImageModal.classList.remove('open'));
      document.getElementById('closeUsers').addEventListener('click', () => document.getElementById('usersModal').classList.remove('open'));
      document.getElementById('closeDeclarantCreateFront').addEventListener('click', () => els.declarantCreateModalFront.classList.remove('open'));
      document.getElementById('closeDeclarantsListFront').addEventListener('click', () => els.declarantsListModalFront.classList.remove('open'));
      document.getElementById('openNewDeclarantFromListFront').addEventListener('click', () => {
        els.declarantsListModalFront.classList.remove('open');
        els.declarantCreateModalFront.classList.add('open');
      });

      [els.detailModal, els.threadModal, els.publishModal, els.publicationsImageModal, document.getElementById('usersModal'), els.declarantCreateModalFront, els.declarantsListModalFront].forEach((modal) => {
    ...
    function bindNavbar() {
      ...
      openPublicationsImageBtn.addEventListener('click', (e) => {
        ...
        els.publicationsSingleImage.src = buildPublicationsSingleImage(rows);
        els.publicationsImageModal.classList.add('open');
      });
