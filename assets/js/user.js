...
const HISTORY_API = "../../View/frontoffice/historique_api.php";
...
function rechercherTrajet() {
  const dep = (safeGetEl('searchDepart')?.value || '').toLowerCase();
  const arr = (safeGetEl('searchArrivee')?.value || '').toLowerCase();
...
function applySort() {
  if (!safeGetEl('resultats')) return;
  const val = (safeGetEl('sortSelect')?.value || '');
  const search = (safeGetEl('tableSearch')?.value || '').toLowerCase();
  ...
  const pageSize = window.pageSize || 8;
  // Recalculate total pages after filtering
  window.totalPages = Math.max(1, Math.ceil(data.length / pageSize));
  window.currentPage = Math.min(Math.max(1, window.currentPage || 1), window.totalPages);
...
function renderTrajets(data) {
  const tbody = safeGetEl('resultats');
  if (!tbody) return;
...
function confirmerReservation() {
  ...
  fetch(DEST_API, {
    ...
    .then(data => {
      ...
      const vehiculeId = parseInt((trip && trip.vehicule_id) ? trip.vehicule_id : 0, 10);
      if (vehiculeId > 0) {
        toast(data.message || 'Destination confirmée. Redirection vers le formulaire...');
        window.location.href =
          'reserver_vehicule.php?vehicule_id=' + encodeURIComponent(vehiculeId) +
          '&trajet_id=' + encodeURIComponent(curResId) +
          '&destination_prix=' + encodeURIComponent(selectedStop.prix) +
...
function chargerTrajets() {
  fetch(API_URL + "?paginate=1&page=1&limit=10000&sort=id_T&order=DESC")  // Load all trips
    ...
    .then(data => {
      ...
      window.currentPage = 1;
      if (safeGetEl('resultats')) applySort();
      if (typeof window.loadAllTrips === 'function' && safeGetEl('allTripsBody')) window.loadAllTrips();
    })
...
window.app.onRouteChange = onRouteChange;
window.app.updateAllArretPrix = updateAllArretPrix;
window.app.updateArretSingle = updateArretSingle;
window.app.rmArret = rmArret;
window.app.ajouterArret = ajouterArret;
window.app.ajouterTrajet = ajouterTrajet;
window.app.modifierTrajet = modifierTrajet;
window.app.supprimerTrajet = supprimerTrajet;
window.app.rechercherTrajet = rechercherTrajet;
window.app.filterTable = filterTable;
window.app.applySort = applySort;
window.app.sortBy = sortBy;
window.app.resetForm = resetForm;
window.app.resetAfterEdit = resetAfterEdit;
window.app.confirmerReservation = confirmerReservation;
window.app.annulerReservation = annulerReservation;