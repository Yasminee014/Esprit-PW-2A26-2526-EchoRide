...
require_once __DIR__ . '/../../Model/User.php';   // on inclut le modèle User
...
// Normaliser les variables de session
if (!isset($_SESSION['admin_id'])) $_SESSION['admin_id'] = 1;
if (!isset($_SESSION['admin_nom'])) $_SESSION['admin_nom'] = 'Admin';

// Charger la photo admin depuis la BDD
if (empty($_SESSION['admin_photo'])) {
    require_once __DIR__ . '/../../Config/Database.php';
    $stmtPhoto = Database::getInstance()->prepare("SELECT photo FROM admins WHERE id = :id");
...
@media (max-width: 600px) {
    ...
    <style>
        ...
        .sidebar-header .logo-tagline { font-size:.65rem; color:#BFD8F1; margin-top:2px; letter-spacing:2px; opacity:.85; font-family:'Poppins',sans-serif; }
        .nav-section { color:#CFE6FF; font-size:.7rem; text-transform:uppercase; letter-spacing:2px; padding:.75rem 1.5rem; margin-top:.5rem; opacity:.8; font-family:'Poppins',sans-serif; font-weight:600; }
        .sidebar nav ul { list-style:none; }
        .sidebar nav ul li { margin-bottom:.25rem; }
        .sidebar nav ul li a { display:flex; align-items:center; gap:12px; padding:.7rem 1.5rem; color:#EAF4FF; text-decoration:none; transition:all .3s; font-size:.85rem; margin:0 .5rem; border-radius:10px; font-family:'Poppins',sans-serif; font-weight:500; }
        ...
        .sidebar nav ul li a.active i { color:#fff; }
        .sidebar-footer { margin-top:auto; padding:1rem 1.5rem; border-top:1px solid rgba(255,255,255,.1); }
        .sidebar-footer a { display:flex; align-items:center; gap:12px; color:#FFCDD2; text-decoration:none; font-size:.85rem; padding:.5rem 0; border-radius:10px; transition:all .3s; font-family:'Poppins',sans-serif; }
...
<body>
    <div class="wrap">
        <aside class="sidebar">
            <div class="sidebar-header">
                <a href="<?= BASE_URL ?>View/backoffice/admin_dashboard.php" class="logo">
                    <img src="<?= BASE_URL ?>assets/images/photo.png" alt="EcoRide Logo" class="logo-img" onerror="this.style.display='none'">
            ...
            <nav>
                <ul>
                    <li><a href="<?= BASE_URL ?>View/backoffice/admin_dashboard.php" class="active"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
                    ...
                    <li><a href="<?= BASE_URL ?>View/backoffice/admin.php"><i class="fas fa-exclamation-triangle"></i> éclamations</a></li>
                    <li><a href="<?= BASE_URL ?>View/backoffice/admin.php"><i class="fas fa-car"></i> Véhicules</a></li>
                    <li><a href="<?= BASE_URL ?>View/backoffice/admin.php"><i class="fas fa-search-location"></i> Objets perdus</a></li>
                </ul>
            </nav>
            <div class="sidebar-footer">
                <a href="<?= BASE_URL ?>Controller/AdminController.php?action=logout"><i class="fas fa-sign-out-alt"></i> Déconnexion</a>
            </div>
        </aside>
    ...
    <div class="chatbot-container" id="chatbotContainer">
        ...
        <div class="chatbot-messages" id="chatbotMessages">
            <div class="message bot">? Bonjour <strong>Admin <?= htmlspecialchars($_SESSION['admin_nom'] ?? '') ?></strong> !<br>Je suis votre assistant. Cliquez sur une cat?gorie ci-dessus pour voir les questions pr?d?finies.</div>
        </div>
        <div class="chatbot-input-area"><input type="text" id="chatbotInput" placeholder="Ou ?crivez votre question..." autocomplete="off"><button id="chatbotSend"><i class="fas fa-paper-plane"></i></button></div>
    </div>
    ...
    aide: {
        ...
        • Proposer ou participer à des trajets<br>
            ...
            window.confirmBanAction = function(id) {
                window.location.href = '<?= BASE_URL ?>Controller/AdminController.php?action=banPassager&id=' + id;
            };
            window.confirmUnbanAction = function(id) {
                window.location.href = '<?= BASE_URL ?>Controller/AdminController.php?action=unbanPassager&id=' + id;
            };
