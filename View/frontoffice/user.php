...
.hero-section.lostfound-hero {
  background: linear-gradient(135deg, #1A6DCC 0%, #165191 100%);
  min-height: 200px;
}
...
.section-switcher .switch-btn:hover {
  background: rgba(97,179,250,.14);
  border-color: rgba(97,179,250,.4);
  transform: translateY(-1px);
}
.section-switcher .switch-btn.active {
  background: rgba(97,179,250,.22);
  border-color: var(--blue-light);
  color: var(--blue-light);
  box-shadow: 0 8px 20px rgba(25,118,210,.18);
}
...
.abtn-map  { background:rgba(97,179,250,.12); color:var(--blue-light); }
.abtn-map:hover  { background:rgba(97,179,250,.28); }
.abtn-car  { background:rgba(255,255,255,.08); color:#d7e6ff; }
.abtn-car:hover  { background:rgba(255,255,255,.18); }
.abtn-car.disabled,
.abtn-car:disabled {
  opacity:.45;
  cursor:not-allowed;
...
<div class="container">
  ...
  <!-- ══════════════════════════════════════════
       TAB 1 — MES TRAJETS
  ══════════════════════════════════════════════ -->
  <div id="tab-mes-trajets" class="page-tab-content <?= $activeTab === 'mes-trajets' ? 'active' : '' ?>">
  ...
  ══════════════════════════════════════════════ -->
  <div id="tab-tous-trajets" class="page-tab-content <?= $activeTab === 'tous-trajets' ? 'active' : '' ?>">
<!-- Recherche IA - Bannière élégante -->
<div style="
...
">
    <div style="display: flex; align-items: stretch; flex-wrap: wrap;">
        ...
        ">
            <div style="
                ...
                font-size: 1.6rem;
            ">
                <i class="fas fa-microchip" style="color: #61B3FA;"></i>
            </div>
            <div>
                <h3 style="margin: 0 0 4px 0; font-size: 1.1rem; font-weight: 600;">
                    Moteur de Matching Intelligent
                </h3>
                <p style="margin: 0; font-size: 0.8rem; color: #A7A9AC;">
                    Algorithme de scoring • Tolérance géographique • Combinaisons optimisées
                </p>
        ...
        <div style="
            ...
            display: flex;
            align-items: center;
            border-left: 1px solid rgba(97, 179, 250, 0.15);
        ">
            <a href="recherche_ia.php" style="
                display: inline-flex;
                align-items: center;
                gap: 12px;
                background: linear-gradient(135deg, #1976D2, #0F3B6E);
            ...
            " onmouseover="this.style.transform='translateX(3px)'; this.style.boxShadow='0 4px 12px rgba(25,118,210,0.4)';"
               onmouseout="this.style.transform='translateX(0)'; this.style.boxShadow='0 2px 8px rgba(25,118,210,0.3)';">
  ...
  ══════════════════════════════════════════════ -->
  <div id="tab-historique" class="page-tab-content <?= $activeTab === 'historique' ? 'active' : '' ?>">
    <div class="section-indicator">
  ...
  <!-- ══════════════════════════════════════════
       TAB 4 — MES FAVORIS
  ══════════════════════════════════════════════ -->
  <div id="tab-favoris" class="page-tab-content <?= $activeTab === 'favoris' ? 'active' : '' ?>">
    <div class="section-indicator">
...
// Ouvrir un onglet specifique si demande via URL (?tab=tous-trajets)
document.addEventListener('DOMContentLoaded', function () {
  const params = new URLSearchParams(window.location.search);
  const requestedTab = params.get('tab');
  if (requestedTab && ['mes-trajets', 'tous-trajets', 'historique', 'favoris', 'mes-objets-perdus'].includes(requestedTab)) {
    navToTab(requestedTab);
  }
...
const FAV_API = '../../Controller/FavoriteController.php';
...
function syncFavoritesFromServer() {
  fetch(FAV_API)
    ...
    .then(resp => {
      ...
      saveFavorites(resp.data);
      if (document.getElementById('tab-favoris')?.classList.contains('active')) {
        renderFavorites();
      }
...
function toggleFavorite(id) {
  ...
  if (idx >= 0) {
    fetch(FAV_API + '?trajet_id=' + encodeURIComponent(id), { method: 'DELETE' }).catch(() => {});
    favs.splice(idx, 1);
    saveFavorites(favs);
...
function loadAllTrips() {
  ...
  tbody.innerHTML = paginatedTrips.map(t => {
    ...
    const nbArrets = (typeof allDests !== 'undefined')
      ...
      : (t.nb_arrets || 0);
    const vehiculeId = parseInt(t.vehicule_id || 0, 10);
    const vehiculeBtn = vehiculeId > 0
      ? `<a class="abtn abtn-car" title="Voir la voiture associée" href="vehicule_details.php?id=${vehiculeId}">
          <i class="fas fa-car-side"></i>
        </a>`
      : `<button class="abtn abtn-car disabled" title="Aucune voiture associée" disabled>
          <i class="fas fa-car-side"></i>
        </button>`;
    ...
    return `<tr>
      ...
      <td><div class="abtns">
        ${favBtnHtml(t.id_T)}
        <button class="abtn abtn-map" title="Voir sur la carte" onclick="window.openTripViewMap && window.openTripViewMap(${t.id_T})">
...
function loadHistorique() {
  ...
  fetch('./historique_api.php')
    .then(r => r.json())
    .then(resp => {
      const allRows = Array.isArray(resp?.data) ? resp.data : [];
      const rows = allRows.filter(r => String(r.statut || '').toLowerCase() === 'confirmee');
      ...
      if (countEl) countEl.textContent = rows.length + ' réservation(s)';
      document.getElementById('hist-total').textContent = rows.length;
      document.getElementById('hist-confirmed').textContent = stats.confirmees || rows.length;
      document.getElementById('hist-cancelled').textContent = stats.annulees || 0;
      document.getElementById('hist-pending').textContent = stats.en_attente || 0;

      if (!pageRows.length) {
        tbody.innerHTML = '<tr><td colspan="7"><div class="empty"><i class="fas fa-history"></i><p>Aucune réservation enregistrée.</p></div></td></tr>';
      } else {
        const badgeClass = s => s === 'confirmee' ? 'badge-confirmed' : (s === 'annulee' ? 'badge-cancelled' : 'badge-pending');
        const badgeLabel = s => s === 'confirmee' ? '✅ Confirmée' : (s === 'annulee' ? '❌ Annulée' : '⏳ En attente');
        tbody.innerHTML = pageRows.map((r, i) => `<tr>
          <td>${start + i + 1}</td>
          <td>${escH(r.point_depart || '—')}<br><small style="color:var(--grey)">${escH(r.marque || '—')} ${escH(r.modele || '')}</small></td>
          <td>${escH(r.point_arrive || '—')}<br><small style="color:var(--grey)">${escH(r.immatriculation || '—')}</small></td>
          <td>${parseFloat(r.prix_total || 0).toFixed(2)} DT</td>
          <td>${parseFloat(r.distance_total || 0) > 0 ? parseFloat(r.distance_total).toFixed(0) + ' km' : '—'}</td>
          <td><strong>${escH(r.arrets_reserves || r.destinations || '—')}</strong></td>
          <td><span class="badge ${badgeClass(r.statut)}">${badgeLabel(r.statut)}</span></td>
        </tr>`).join('');
      }

      if (typeof window.renderPagination === 'function') {
        window.totalPages = Math.max(1, Math.ceil(rows.length / pageSize));
        window.renderPagination('pagination-container-historique');
      }
    })
    .catch(() => {
      tbody.innerHTML = '<tr><td colspan="7"><div class="empty"><i class="fas fa-triangle-exclamation"></i><p>Erreur chargement historique.</p></div></td></tr>';
    });
}
...
<script src="../../assets/js/user.js?v=<?= urlencode((string)@filemtime(__DIR__ . '/../../assets/js/user.js')) ?>"></script>
<script src="../../assets/js/map-integration.js?v=<?= urlencode((string)@filemtime(__DIR__ . '/../../assets/js/map-integration.js')) ?>"></script>
...
const appFallbackKeys = [
  'switchMode', 'onRouteChange', 'updateAllArretPrix', 'updateArretSingle',
  'rmArret', 'ajouterArret', 'ajouterTrajet', 'modifierTrajet',
  'supprimerTrajet', 'rechercherTrajet', 'filterTable', 'applySort',
  'sortBy', 'resetForm', 'resetAfterEdit', 'confirmerReservation',
  'annulerReservation', 'reserverTrajetAllTrips', 'selectStop',
  'confirmerReservationAll', 'annulerReservationAll', 'renderFavorites',
  'toggleFavorite', 'isFavorite', 'favBtnHtml'
];
for (const key of appFallbackKeys) {
  if (!window.app[key] && typeof window[key] === 'function') {
    window.app[key] = window[key];
  }
}
...
window.app.confirmerReservationAll = function() {
  if (!selectedStop) {
    showToast('Choisissez un point de descente !', false);
    return;
  }
  if (!curResId) {
    showToast('Aucun trajet sélectionné', false);
    return;
  }

  // Récupérer les paramètres de l'URL
  const params = new URLSearchParams(window.location.search);
  const vehiculeIdFromUrl = parseInt(params.get('vehicule_id') || '0', 10);
  
  // Récupérer les infos du trajet
  const trip = allTrips.find(t => String(t.id_T) === String(curResId));
  const vehiculeIdFromTrip = (trip && trip.vehicule_id) ? parseInt(trip.vehicule_id, 10) : 0;
  const vehiculeId = vehiculeIdFromUrl > 0 ? vehiculeIdFromUrl : vehiculeIdFromTrip;
  
  // Afficher un message de chargement
  showToast('Redirection vers la réservation...', true);
  
  // Redirection directe vers la page de réservation
  if (vehiculeId > 0) {
    window.location.href = 'reserver_vehicule.php?vehicule_id=' + vehiculeId + 
                          '&trajet_id=' + curResId + 
                          '&destination_prix=' + selectedStop.prix +
                          '&destination_nom=' + encodeURIComponent(selectedStop.nom);
  } else {
    window.location.href = 'reserver_vehicule.php?trajet_id=' + curResId + 
                          '&destination_prix=' + selectedStop.prix +
                          '&destination_nom=' + encodeURIComponent(selectedStop.nom);
  }
};
