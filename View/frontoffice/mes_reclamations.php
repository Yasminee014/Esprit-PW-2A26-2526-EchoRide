...
.header-left{display:flex;flex-direction:column;gap:.8rem;}
.lang-form{display:flex;align-items:center;gap:.75rem;}
.lang-form label{color:#ccc;font-size:.88rem;}
.lang-form select{min-width:150px;padding:.7rem .9rem;border-radius:999px;border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.1);color:#fff;font-size:.95rem;cursor:pointer;outline:none;transition:all .2s;}
.lang-form select:hover{background:rgba(255,255,255,.15);}
.lang-form select option{background:#0A1628;color:#fff;}
...
.hero-small{background:linear-gradient(135deg,#1976D2,#0F3B6E);border-radius:20px;padding:1.5rem 2rem;margin-bottom:2rem;display:flex;justify-content:space-between;align-items:center;}
.hero-small h2{font-size:1.5rem;margin-bottom:.3rem;font-weight:700;color:#fff;}
.hero-small h2 i{color:#fff;}
.hero-small p{color:rgba(255,255,255,.8);font-size:.85rem;}
.hero-small-icon{font-size:3rem;opacity:.25;color:#fff;}
</style>
</head>
<body>
<div class="container">
...
<div class="hero-small">
    ...
    <div class="hero-small-icon"><i class="fas fa-exclamation-triangle"></i></div>
</div>

<div class="header-flex">
    <div class="header-left">
        <form class="lang-form" method="get" action="">
            <label for="languageSelect" class="sr-only">Sélectionnez la langue</label>
            <select id="languageSelect" name="lang" onchange="this.form.submit()">
                <option value="fr" <?= $selected_language === 'fr' ? 'selected' : '' ?>>Français</option>
                <option value="en" <?= $selected_language === 'en' ? 'selected' : '' ?>>English</option>
            </select>
        </form>
    </div>
    <a href="/ecoride/View/frontoffice/index.php?switch=1" class="btn-switch">
