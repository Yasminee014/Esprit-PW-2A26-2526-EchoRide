...
// Recharger les données utilisateur depuis la BDD si les clés de session sont manquantes
if (!isset($_SESSION['user_prenom']) || !isset($_SESSION['user_nom']) || !isset($_SESSION['user_email'])) {
    require_once __DIR__ . '/../../Config/Database.php';
    $db = Database::getInstance();
    $stmt = $db->prepare("SELECT prenom, nom, email, role, telephone, photo, avatar, avatar_options FROM users WHERE id = :id");
    ...
    if ($row) {
        ...
        $_SESSION['user_role']      = $row['role'] ?? 'passager';
        $_SESSION['user_telephone'] = $row['telephone'] ?? '';
        $_SESSION['user_photo']     = $row['photo'] ?? '';
        $_SESSION['user_avatar']    = $row['avatar'] ?? '';
        if (!empty($row['avatar_options'])) {
            $decoded = json_decode($row['avatar_options'], true);
            $_SESSION['user_avatar_options'] = is_array($decoded) ? $decoded : [];
        }
    }
}
...
<div class="profile-container">
    <div class="profile-header">
        ...
        <div class="avatar-btns">
            <form method="POST" action="<?= BASE_URL ?>Controller/UserController.php?action=uploadPhoto" enctype="multipart/form-data" style="display:inline;">
    ...
    <div class="profile-grid">
        ...
        <div class="card">
            ...
            <?php endif; ?>
            <form method="POST" action="<?= BASE_URL ?>Controller/UserController.php?action=updateProfile">
        ...
        <div class="card">
            ...
            <?php endif; ?>
            <form method="POST" action="<?= BASE_URL ?>Controller/UserController.php?action=changePassword">
                <div class="form-group">
    ...
    <a href="<?= BASE_URL ?>Controller/UserController.php?action=dashboard" class="btn-back-dashboard">
...
<script src="<?= BASE_URL ?>View/frontoffice/js/profile.validation.js"></script>
<script>
// ── Dropdown profil ───────────────────────────────────────
...
function saveAvatar() {
    ...
    fetch('<?= BASE_URL ?>Controller/UserController.php?action=saveAvatar', {
        method: 'POST',
