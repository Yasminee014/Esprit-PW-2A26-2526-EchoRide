...
} elseif ($showForm === 'showLogin' || $showForm === 'login' || $showForm === 'landing') {
    $showRegister = false;
    $showLogin = true;
    $showForm = 'showLogin';
} else {
...
<?php if ($showLogin): ?>
  ...
  <div class="form-card" id="loginFormCard">
    ...
    <form method="POST" id="loginFormElement" action="<?= BASE_URL ?>Controller/UserController.php?action=login" novalidate>
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.2rem; gap:0.5rem;">
        ...
        <a href="<?= BASE_URL ?>View/frontoffice/forgot_password.php" class="forgot-link" style="white-space:nowrap;">
    ...
    <p style="text-align:center; color:#A7A9AC; font-size:0.85rem;">
      Pas encore inscrit ?
      <a href="<?= BASE_URL ?>Controller/UserController.php?action=showRegister" class="switch-link">Créer un compte</a>
    </p>
  </div>
...
<div class="form-section">
    <div class="form-container">
        ...
        <div class="form-card" id="registerForm">
            ...
            <form method="POST" id="registerFormElement" action="<?= BASE_URL ?>Controller/UserController.php?action=register" novalidate>
            ...
            <p style="text-align:center; color:#A7A9AC; font-size:0.85rem;">
                Déjà inscrit ? 
                <a href="<?= BASE_URL ?>Controller/UserController.php?action=showLoginForm" class="switch-link">Se connecter</a>
            </p>
        </div>
</div>
</div>
...
<script src="<?= BASE_URL ?>View/frontoffice/js/login.validation.js"></script>
...
async function loadProfileDescriptor(email) {
    const res  = await fetch('<?= BASE_URL ?>Controller/UserController.php?action=getFaceImage', {
        method: 'POST',
...
function submitPendingForm() {
    ...
    realForm.action = '<?= BASE_URL ?>Controller/UserController.php?action=login';
