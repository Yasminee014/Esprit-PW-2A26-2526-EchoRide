...
$spaTabs = ['passagers', 'trajets', 'destinations', 'reclamations', 'vehicules', 'lost_found'];
$sidebarActive = ($tab !== '' && in_array($tab, $spaTabs, true)) ? $tab : 'dashboard';
...
<body>
    <div style="display: flex; width:100%; overflow-x:hidden;">
        <?php require_once __DIR__ . '/partials/partials.php'; sidebar_spa($sidebarActive); ?>
    ...
    <script>
        ...
        function showPage(page) {
            allPages.forEach(p => { const el = document.getElementById(p + 'Page'); if (el) el.style.display = (p === page) ? 'block' : 'none'; });
            document.querySelectorAll('.sidebar nav ul li a').forEach(a => {
                a.classList.remove('active');
                const href = a.getAttribute('href') || '';
                if (page === 'dashboard') {
                    if (href.includes('action=dashboard') && !href.includes('tab=')) a.classList.add('active');
                } else if (href.includes('tab=' + page)) {
                    a.classList.add('active');
                }
            });
            if (page === 'passagers') setTimeout(() => resetAndDisplay(), 50);
        }
