...
<title>AI Helper - Eco Ride Admin</title>
...
.chat-header { padding:16px 24px; background:linear-gradient(135deg,rgba(25,118,210,0.2),rgba(97,179,250,0.1)); border-bottom:1px solid rgba(255,255,255,0.08); display:flex; align-items:center; gap:12px; }
.chat-header .ai-avatar { width:42px; height:42px; border-radius:50%; background:linear-gradient(135deg,#1976D2,#42A5F5); display:flex; align-items:center; justify-content:center; font-size:1.2rem; box-shadow:0 0 15px rgba(25,118,210,0.4); }
...
.msg.user .msg-avatar { background:linear-gradient(135deg,#1976D2,#42A5F5); color:#fff; }
.msg.ai   .msg-avatar { background:linear-gradient(135deg,#1976D2,#0F3B6E); color:#fff; }
.msg-bubble { padding:10px 14px; border-radius:14px; font-size:0.88rem; line-height:1.55; max-width:100%; word-break:break-word; }
.msg.user .msg-bubble { background:linear-gradient(135deg,#1976D2,#42A5F5); color:#fff; border-bottom-right-radius:4px; }
...
.msg.ai   .msg-bubble code { background:rgba(25,118,210,0.2); color:#61B3FA; padding:1px 6px; border-radius:4px; }
...
.typing-dots span { width:7px; height:7px; border-radius:50%; background:#1976D2; animation:dot 1.2s infinite; }
...
.voice-bar.speaking { background:rgba(25,118,210,0.08); border-color:rgba(25,118,210,0.2); color:#61B3FA; }
...
#micBtn.speak  { background:rgba(25,118,210,0.15); color:#61B3FA; border-color:#1976D2; }
...
#chatInput:focus { border-color:rgba(25,118,210,0.5); }
#chatInput::placeholder { color:#555; }
#sendBtn { width:42px; height:42px; border-radius:12px; border:none; background:linear-gradient(135deg,#1976D2,#42A5F5); color:#fff; cursor:pointer; font-size:0.95rem; flex-shrink:0; transition:transform 0.2s,box-shadow 0.2s; }
#sendBtn:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 6px 20px rgba(25,118,210,0.4); }
#sendBtn:disabled { opacity:0.4; cursor:not-allowed; }

.suggestions { padding:10px 16px 0; display:flex; flex-wrap:wrap; gap:7px; }
.sug-chip { background:rgba(25,118,210,0.1); border:1px solid rgba(25,118,210,0.3); color:#61B3FA; padding:5px 12px; border-radius:20px; font-size:0.78rem; cursor:pointer; transition:background 0.2s; user-select:none; }
.sug-chip:hover { background:rgba(25,118,210,0.25); }

.users-panel { width:340px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:16px; overflow:hidden; display:flex; flex-direction:column; }
.cp-header { padding:16px 20px; border-bottom:1px solid rgba(255,255,255,0.08); background:rgba(25,118,210,0.05); }
.cp-header h4 { margin:0; color:#e0e0e0; font-size:0.95rem; }
.cp-header p  { margin:3px 0 0; color:#888; font-size:0.75rem; }
.users-list { flex:1; overflow-y:auto; padding:10px; display:flex; flex-direction:column; gap:8px; scrollbar-width:thin; }
.user-card { background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:10px; padding:10px 13px; transition:border 0.2s; }
.user-card:hover { border-color:rgba(25,118,210,0.4); }
.user-card .cn { color:#e0e0e0; font-size:0.87rem; font-weight:600; }
.user-card .ce { color:#888; font-size:0.75rem; margin-top:2px; }
.user-card .ci { color:#555; font-size:0.72rem; margin-top:2px; }
.badge-actif   { background:rgba(0,230,118,0.15); color:#00e676; padding:2px 8px; border-radius:10px; font-size:0.68rem; float:right; }
.badge-inactif { background:rgba(255,82,82,0.15);  color:#ff5252; padding:2px 8px; border-radius:10px; font-size:0.68rem; float:right; }
.empty-list { text-align:center; color:#555; padding:40px 20px; font-size:0.85rem; }
</style>
</head>
<body>

<?php sidebar_compact('ai_helper'); ?>

<main class="main" style="padding:20px;">
    <div class="top-bar" style="margin-bottom:18px;">
        <h1 style="color:#e0e0e0;"><i class="fas fa-robot" style="color:#1976D2;"></i> AI Helper — Gestion Utilisateurs</h1>
        <span style="background:rgba(25,118,210,0.2);color:#61B3FA;padding:6px 14px;border-radius:20px;font-size:0.82rem;">
            <i class="fas fa-user-shield"></i> <?= htmlspecialchars($_SESSION['admin_nom'] ?? '') ?>
        </span>
    </div>
    ...
    <div class="ai-wrap">
        ...
        <div class="chat-panel">
                ...
                <div class="msg ai">
                    ...
                    <div>
                        <div class="msg-bubble">
                            Bonjour ! Je suis <strong>AI Helper</strong>, votre assistant Eco Ride.<br><br>
            ...
            <div class="suggestions" id="suggs">
                <span class="sug-chip" onclick="useSugg(this)">📋 Liste tous les utilisateurs</span>
                <span class="sug-chip" onclick="useSugg(this)">🔍 Cherche un utilisateur</span>
                <span class="sug-chip" onclick="useSugg(this)">🔃 Trie par nom</span>
                <span class="sug-chip" onclick="useSugg(this)">🔃 Trie par statut</span>
                <span class="sug-chip" onclick="useSugg(this)">➕ Ajouter un utilisateur</span>
                <span class="sug-chip" onclick="useSugg(this)">🚫 Bloquer un utilisateur</span>
        ...
        <div class="users-panel">
            <div class="cp-header">
                <h4><i class="fas fa-users" style="color:#1976D2;margin-right:8px;"></i>Utilisateurs <span id="uCount" style="color:#61B3FA;"></span></h4>
...
function renderUsers(list) {
    ...
    if (!list.length) { usersList.innerHTML = '<div class="empty-list"><i class="fas fa-users-slash"></i><br>Aucun utilisateur</div>'; return; }
    usersList.innerHTML = list.map(function(u) {
        const badge = u.statut === 'actif'
            ? '<span class="badge-actif">actif</span>'
            : '<span class="badge-inactif">inactif</span>';
        return '<div class="user-card">' +
            '<div><span style="color:#555;font-size:0.7rem;">#' + u.id + '</span>' + badge + '</div>' +
            '<div class="cn">' + u.prenom + ' ' + u.nom + '</div>' +
            '<div class="ce"><i class="fas fa-envelope" style="width:13px;"></i> ' + u.email + '</div>' +
            (u.telephone ? '<div class="ci"><i class="fas fa-phone" style="width:13px;"></i> ' + u.telephone + '</div>' : '') +
            '<div class="ci" style="color:#1976D2;"><i class="fas fa-tag" style="width:13px;"></i> ' + (u.role || 'passager') + '</div>' +
            '</div>';
    }).join('');
}

function renderTable(list) {
    if (!list.length) { addMsg('ai', 'Aucun utilisateur trouvé.'); return; }
    const rows = list.map(function(u) {
        var badge = u.statut === 'actif'
            ...
            : '<span style="background:rgba(255,82,82,0.15);color:#ff5252;padding:2px 7px;border-radius:8px;font-size:0.7rem;">inactif</span>';
        return '<tr>' +
            '<td style="padding:5px 8px;color:#aaa;">#' + u.id + '</td>' +
            '<td style="padding:5px 8px;color:#e0e0e0;font-weight:600;">' + u.prenom + ' ' + u.nom + '</td>' +
            '<td style="padding:5px 8px;color:#888;font-size:0.78rem;">' + u.email + '</td>' +
            '<td style="padding:5px 8px;color:#888;font-size:0.78rem;">' + (u.telephone || '-') + '</td>' +
            '<td style="padding:5px 8px;color:#1976D2;font-size:0.78rem;">' + (u.role || '-') + '</td>' +
    ...
    const tableHtml = '<div style="overflow-x:auto;margin-top:6px;">' +
        ...
        '<th style="padding:5px 8px;color:#61B3FA;text-align:left;">ID</th>' +
        '<th style="padding:5px 8px;color:#61B3FA;text-align:left;">Nom</th>' +
        '<th style="padding:5px 8px;color:#61B3FA;text-align:left;">Email</th>' +
        '<th style="padding:5px 8px;color:#61B3FA;text-align:left;">Tél</th>' +
        '<th style="padding:5px 8px;color:#61B3FA;text-align:left;">Rôle</th>' +
        '<th style="padding:5px 8px;color:#61B3FA;text-align:left;">Statut</th>' +
...
async function sendMsg() {
    ...
    showTyping();
    try {
        var res  = await fetch('<?= BASE_URL ?>controllers/AIController.php?action=chat', {
            method: 'POST',
        ...
        addMsg('ai', reply);
        speakText(reply);
        if (data.clients !== undefined) renderUsers(data.clients);
...
(async function() {
    try {
        ...
        var data = await res.json();
        if (data.clients) renderUsers(data.clients);
    } catch (e) {
        usersList.innerHTML = '<div class="empty-list">Erreur de chargement</div>';
    }
})();
...
if (!SpeechRec) { micBtn.disabled=true; micBtn.style.opacity='0.3'; micBtn.title='Non supporté — utilisez Chrome ou Edge'; }

function toggleMic() { if (!SpeechRec) return; listening ? stopVoice() : startVoice(); }
...
function startVoice() {
    ...
    recog.onstart = function() {
        listening=true; micBtn.className='listen'; micBtn.innerHTML='<i class="fas fa-stop"></i>';
        voiceBar.className='voice-bar show'; voiceBarText.textContent='Écoute en cours... parlez !';
    };
    recog.onresult = function(e) {
        var txt='';
        for(var i=e.resultIndex;i<e.results.length;i++) txt+=e.results[i][0].transcript;
        chatInput.value=txt; chatInput.dispatchEvent(new Event('input'));
        voiceBarText.textContent=txt;
        if(e.results[e.results.length-1].isFinal) setTimeout(function(){sendMsg();},400);
    };
    recog.onerror = function(e) {
        var m={'no-speech':'Aucune voix détectée.','not-allowed':'Microphone bloqué.','audio-capture':'Micro introuvable.','network':'Erreur réseau.'};
        voiceBarText.textContent=m[e.error]||('Erreur: '+e.error); stopVoice();
    };
...
function stopVoice() {
    listening=false; micBtn.className=''; micBtn.innerHTML='<i class="fas fa-microphone"></i>';
    voiceBar.className='voice-bar'; if(recog){try{recog.stop();}catch(e){}}
}
...
function speakText(text) {
    ...
    utt.onstart = function() {
        micBtn.className='speak'; micBtn.innerHTML='<i class="fas fa-volume-up"></i>';
        voiceBar.className='voice-bar show speaking'; voiceBarText.textContent='AI Helper parle...';
    };
    utt.onend = utt.onerror = function() {
        micBtn.className=''; micBtn.innerHTML='<i class="fas fa-microphone"></i>';
